...
extern "C"
{
    ...
    extern GLuint stbgl_create_program(char const **vertex_source, char const **frag_source, char const **binds,
                                       char *error, int error_buflen);
    ...
    //             char **vertex_source,   // NULL or one or more strings with the vertex shader source, with a final
    //             NULL char **frag_source,     // NULL or one or more strings with the fragment shader source, with a
    //             final NULL char **binds,           // NULL or zero or more strings with attribute bind names, with a
    //             final NULL char *error,            // output location where compile error message is placed int
    ...
    extern GLuint stbgl_compile_shader(GLenum type, char const **sources, int num_sources, char *error,
                                       int error_buflen);
    ...
    extern GLuint stbgl_link_program(GLuint vertex_shader, GLuint fragment_shader, char const **binds, int num_binds,
                                     char *error, int error_buflen);
...
STB_GLPROG_DECLARE GLuint stbgl_compile_shader(GLenum type, char const **sources, int num_sources, char *error,
                                               int error_buflen)
{
    ...
    if (!shader)
    {
        stb_glprog_error(error, error_buflen, "Couldn't allocate shader object in stbgl_compile_shader for ", typename,
                         NULL);
        return 0;
    }
...
STB_GLPROG_DECLARE GLuint stbgl_link_program(GLuint vertex_shader, GLuint fragment_shader, char const **binds,
                                             int num_binds, char *error, int error_buflen)
{
    int len;
    GLint result;
...
STB_GLPROG_DECLARE GLuint stbgl_create_program(char const **vertex_source, char const **frag_source, char const **binds,
                                               char *error, int error_buflen)
{
...
STB_GLPROG_DECLARE void stbglVertexAttribPointer(GLuint index, GLint size, GLenum type, GLboolean normalized,
