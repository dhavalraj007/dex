    ...
    int run()
    {
        ...
        for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i)
        {
            ...
            glm::vec<L, T> const Result1 =
                glm::prevMultiple(glm::vec<L, T>(Data[i].Source), glm::vec<L, T>(Data[i].Multiple));
    ...
    int run()
    {
        ...
        for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i)
        {
            glm::vec<L, T> const Result0 =
                glm::nextMultiple(glm::vec<L, T>(Data[i].Source), glm::vec<L, T>(Data[i].Multiple));
            Error += glm::vec<L, T>(Data[i].Return) == Result0 ? 0 : 1;
...
namespace findNSB
{
    ...
    int run()
    {
        type<T> const Data[] = {{0x00, 1, -1}, {0x01, 2, -1}, {0x02, 2, -1}, {0x06, 3, -1}, {0x01, 1, 0},
        ...
        for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i)
        {
            glm::vec<L, int> const Result0 = glm::findNSB<L, T, glm::defaultp>(
                glm::vec<L, T>(Data[i].Source), glm::vec<L, int>(Data[i].SignificantBitCount));
