        ...
        */
...
#if (GLM_ARCH & GLM_ARCH_SSE2_BIT && !(GLM_COMPILER & GLM_COMPILER_GCC))
        {
            ...
            for (std::size_t i = 0; i < SimdParam.size(); ++i)
                SimdParam[i] = _mm_set_epi32(static_cast<int>(i % static_cast<std::size_t>(x_max)), 0,
...
namespace bitfieldInterleave5
{
    ...
    GLM_FUNC_QUALIFIER glm::u8vec4 bitfieldDeinterleave_u8vec4(glm::uint32 InterleavedBitfield)
    {
        ...
        return glm::u8vec4((Result >> 0) & 0x000000000000FFFFull, (Result >> 16) & 0x00000000FFFF0000ull,
                           (Result >> 32) & 0x0000FFFF00000000ull, (Result >> 48) & 0xFFFF000000000000ull);
    }
            ...
            for (glm::size_t j = 0; j < 256; ++j)
                for (glm::size_t i = 0; i < 256; ++i)
                    Error += Result[j * 256 + i] ==
                                     glm::bitfieldInterleave(glm::uint8(i), glm::uint8(j), glm::uint8(i), glm::uint8(j))
                                 ? 0
                                 : 1;
    ...
    int perf_new_u8vec4(std::vector<glm::uint32> &Result)
    {
        ...
        for (glm::size_t k = 0; k < 10000; ++k)
            for (glm::size_t j = 0; j < 256; ++j)
                for (glm::size_t i = 0; i < 256; ++i)
                    Error += Result[j * 256 + i] == bitfieldInterleave_u8vec4(glm::uint8(i), glm::uint8(j),
